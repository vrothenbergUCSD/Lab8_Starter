...
describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const textIsRemove = (textValue == 'Remove from Cart'); 
    expect(textIsRemove).toBe(true);
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 1; i < prodItems.length; i++) {
      ...
      button = await shadowRoot.asElement().$('button');
      await button.click();
      innerText = await button.getProperty('innerText');
      textValue = innerText['_remoteObject'].value;
      if (textValue != 'Remove from Cart') {
        console.log(`Not Remove from Cart: ${textValue}`);
        allSayRemove = false;
      }
    }
    ...
    innerText = await cart.getProperty('innerText');
    const count = innerText['_remoteObject'].value;
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const textValue = innerText['_remoteObject'].value;
      if (textValue != 'Remove from Cart') {
        buttonsSayRemove = false;
      }
    }    

    expect(buttonsSayRemove).toBe(true);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    expect(buttonsSayAdd).toBe(true);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    console.log(`Checking localStorage cart ${cart}`);
